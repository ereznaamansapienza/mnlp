...
class SFTDatasetBuilder:
    ...
    def _get_oracle_first_chunks(self, row) -> List[str]:
        """Correct chunk is placed first, followed by distractors."""
        correct_idx = int(row.answer_pos)
        correct_chunk = row.candidate_chunks[correct_idx]

        distractor_indices = [
            i for i in row.retrieved_chunks
            if i != correct_idx
        ][:self.retrieval_store.top_k - 1]

        distractors = [row.candidate_chunks[i] for i in distractor_indices]
        return [correct_chunk] + distractors

    def _get_oracle_random_chunks(self, row) -> List[str]:
        """Correct chunk is placed at a random position."""
        chunks = self._get_oracle_first_chunks(row)
        self.rng.shuffle(chunks)
        return chunks

    def _get_no_answer_chunks(self, row) -> List[str]:
        """Only distractor chunks — correct chunk deliberately excluded."""
        correct_idx = int(row.answer_pos)
        distractor_indices = [
            i for i in row.ranking        # full ranking, not just top_k
            if i != correct_idx
        ][:self.retrieval_store.top_k]
        return [row.candidate_chunks[i] for i in distractor_indices]
    ...
    def _build_example(self, row, chunks: List[str], response: str) -> Dict:
    ...
    def build_sft_examples(
        ...
        no_answer_ratio: float = 0.3,   # fraction of rows to use for no_answer
    ) -> List[Dict]:
        """
        For each row build up to 3 training examples depending on `variants`:
          - oracle_first   : correct chunk first  → gold answer
          - oracle_random  : correct chunk shuffled → gold answer
          - no_answer      : only distractors      → "I don't have that information."

        no_answer_ratio controls the fraction of rows used for the no_answer
        variant (avoids flooding the dataset with abstention examples).
        """
        if not isinstance(df, pd.DataFrame):
            df = df.to_pandas()

        # rows eligible for no_answer variant (sampled)
        no_answer_ids = set(
        ...
        for row in df.itertuples():
            ...
            if "oracle_first" in variants:
                chunks = self._get_oracle_first_chunks(row)
                ex = self._build_example(row, chunks, self._get_answer(row))
                ex["variant"] = "oracle_first"
                examples.append(ex)

            if "oracle_random" in variants:
                chunks = self._get_oracle_random_chunks(row)
                ex = self._build_example(row, chunks, self._get_answer(row))
                ex["variant"] = "oracle_random"
                examples.append(ex)

            if "no_answer" in variants and q_id in no_answer_ids:
                chunks = self._get_no_answer_chunks(row)
                ex = self._build_example(row, chunks, self._get_no_answer_response())
                ex["variant"] = "no_answer"
                examples.append(ex)
    ...
    def build_hf_dataset(
        ...
        """
        Builds a HuggingFace Dataset with a single `text` column
        formatted as a chat-template instruction+response pair,
